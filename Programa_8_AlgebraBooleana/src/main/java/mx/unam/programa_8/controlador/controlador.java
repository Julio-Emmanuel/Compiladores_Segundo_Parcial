package mx.unam.programa_8.controlador;
...
import mx.unam.programa_8.modelo.*;

public class controlador {


        @FXML private TextField txtInput;
        @FXML private TextFlow flowArchivo;
        @FXML private TextFlow flowResultado;
        ...
        private void validarSintaxis(String expresion) {
            try {
                modelo.Lexer lexer = new modelo.Lexer(expresion);
                modelo.Parser parser = new modelo.Parser(lexer.getTokens());


                parser.s();

                mostrarResultado(expresion, "Sintaxis Correcta", Color.GREEN);
            } catch (Exception e) {

                mostrarResultado(expresion, "Error: " + e.getMessage(), Color.RED);
            }
        }

        private void mostrarResultado(String exp, String msg, Color color) {
            Text tExp = new Text(exp + "  =>  ");
            Text tMsg = new Text(msg + "\n");
            tMsg.setFill(color);
            flowResultado.getChildren().addAll(tExp, tMsg);
        }
